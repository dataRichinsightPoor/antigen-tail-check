...
The build job runs `npm test` under Node.js 22. Its pretest step regenerates `web/worker-source.js` from the shared numerical core and worker entry point, then the numerical tests run. A generated-file diff check rejects an out-of-date committed worker bundle.
...
The `v0.1.0-alpha` tag freezes the initial tested implementation. GitHub Pages follows `main`, which may contain later deployment and documentation changes; a live page is not an immutable release artifact.
